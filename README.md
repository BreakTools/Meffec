...
**Infrastructure for realtime scriptable app triggered show control.**
...
```
docker run -p {port}:{port} -e WEBSOCKET_TOKEN="{your_made_up_token}" -e PORT="{port}" -t ghcr.io/breaktools/meffec-server
```
